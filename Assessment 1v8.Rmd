...
# Create a month year
dd$Date <- format(as.Date(dd$Date), "%Y-%m")
```

```{r, include=FALSE}
# Change format
dd$hour <- as.factor(dd$hour)
dd$Date <- as.Date(as.yearmon(dd$Date, "%Y-%m"))
...
# Plot the graph 
ggplot(dd, aes(x=Date))+geom_line(stat="count") + theme_minimal() + theme(axis.title.x=element_blank()) + theme(axis.title.y=element_blank())
```
...
```{r, fig3, fig.cap="Crimes per hour", echo=FALSE, out.extra=''}
# Plot the graph 
...
```{r fig8, echo=FALSE, fig.cap="Crimes per district", out.extra='',fig.align='center'}
